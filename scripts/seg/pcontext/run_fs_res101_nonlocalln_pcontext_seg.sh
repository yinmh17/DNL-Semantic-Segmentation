...
CONFIG_FILE='configs/seg/pcontext/NLln59c_fcn_pcontext_seg.conf'
MAX_ITERS=30000
...
if [[ "$1"x == "train"x ]]; then
  ${DIST_PYTHON} main.py --config_file ${CONFIG_FILE} --phase train --train_batch_size 4 --val_batch_size 1 \
                         --backbone ${BACKBONE} --model_name ${MODEL_NAME} --base_lr ${BASE_LR} --gpu 0 1 2 3 --drop_last y --syncbn y --dist y \
